...
public class ValkyrieAccessor implements TweenAccessor<Valkyrie> {
	...
	public static final int VALK_STATE = 4;
	// TweenAccessor implementation

	@Override
	public int getValues(Valkyrie target, int tweenType, float[] returnValues) {
		switch (tweenType) {
	...
	@Override
	public void setValues(Valkyrie target, int tweenType, float[] newValues) {
		switch (tweenType) {
		...
		case POSITION_XY:
			...
			break;
		case VALK_STATE:target.setState((int) newValues[0]);
